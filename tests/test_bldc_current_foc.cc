...
TEST_F(BLDCFixture, ConstTorque)
{
    ...
    for (int i = 0; i < 1000*10; i++)
    {
        ...
        m_bldc.sampleCurrentHandler(m_voltage_shunt, PICA_DRIVE_CURRENT_MEASURE_PERIOD);
        
        m_bldc.do_checks();

        m_bldc.setTorque(m_torque);

        m_bldc.update((hrt_absnano)(m_ts_now * 1e9));

        m_bldc.sampleCurrentCalibratorHandler(NULL, (hrt_absnano)(m_ts_now * 1e9));
