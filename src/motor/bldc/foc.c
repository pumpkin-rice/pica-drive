...
{
    foc->id_meas = foc->iq_meas = 0.f;
    foc->id_sp = foc->iq_sp = 0.f;
    foc->vd_sp = foc->vq_sp = 0.f;

    foc->i_alpha_meas = foc->i_beta_meas = 0.f;
    foc->v_alpha_final = foc->v_beta_final = 0.f;
...
int foc_run(current_controller_obj *obj, float time2last_sampling, float time2next_output)
{
    ...
    foc->i_alpha_meas = i_alpha_meas;
    foc->i_beta_meas = i_beta_meas;
    ...
    if (isfinite(i_alpha_meas) && isfinite(i_beta_meas)) {
        ...
        if (isfinite(i_alpha_meas) && isfinite(i_beta_meas)) {
            ...
            foc->id_meas +=
                    foc->idq_meas_filter_k * (id - foc->id_meas);
            foc->iq_meas +=
                    foc->idq_meas_filter_k * (iq - foc->iq_meas);
        }
    ...
    if (foc->current_loop_enabled) {
        ...
        mod_vd = vbus2mod * (foc->vd_sp
            + id_err * pi_d->kp + pi_d->integral);
        mod_vq = vbus2mod * (foc->vq_sp
    ...
    // 		&bldc->duty_cycle[0], &bldc->duty_cycle[1], &bldc->duty_cycle[2])
    // ) {
    ...
    foc->v_alpha_final = mod2vbus * mod_valpha;
    foc->v_beta_final  = mod2vbus * mod_vbeta;
